...
public final class MySparkRun {
    ...
    public static void run() {
        ...
        JavaRDD<String> lines = sc.textFile(inputHdfsPath, 5); //java.lang.String path, int minSplits
...
//        ParseProtobuf parseExecutor = new ParseProtobuf();
        JavaRDD<String> results = lines.map(s -> ParseProtobuf.ParseProtobufStruct(s));
//        JavaRDD<String> results = lines;
        // 保存函数，数据输出，spark为结果输出提供了很多接口
        // lines.saveAsNewAPIHadoopFile();
        // lines.saveAsHadoopFile();
        results.repartition(5).saveAsTextFile(outputHdfsPath);
        sc.close();
    }
